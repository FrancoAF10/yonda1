        ...
        <div class="tab-pane fade show active" id="persona" role="tabpanel" aria-labelledby="persona-tab">
            ...
            <div class="tab-pane fade" id="asignacion" role="tabpanel" aria-labelledby="asignacion-tab">
                <div class="container py-4">
                    ...
                    <form id="form-cargafamiliar" enctype="multipart/form-data" class="card shadow-sm p-4 border-0 rounded-3">
                    <input type="hidden" name="idpersona" id="idpersona" value="<?= $persona['idpersona'] ?>">

                    <div class="mb-3">
                        <h5 class="fw-bold text-primary">
                        COLABORADOR: <?= $persona['nombres'] ?> <?= $persona['apepaterno'] ?> <?= $persona['apematerno'] ?>
                        </h5>
                    </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="form-check form-switch mt-3">
                            <input class="form-check-input" type="checkbox" id="estudia" name="estudia">
                            <label class="form-check-label" for="estudia">¿Estudia actualmente?</label>
                        </div>
                        </div>
                        ...
                        <div class="col-md-3">
                        <div class="form-check form-switch mt-3">
                            <input class="form-check-input" type="checkbox" id="dependiente" name="dependiente" checked>
                            <label class="form-check-label" for="dependiente">¿Es dependiente?</label>
                        </div>
                        </div>
                        ...
                        <div class="col-md-6">
                        <div class="form-floating">
                            <input type="file" name="evidencia" id="evidencia" class="form-control" accept=".pdf,.jpg,.jpeg,.png">
                            <label for="evidencia">Evidencia (PDF o imagen)</label>
                        </div>
                        </div>
                    </div>
                    ...
                    <div class="card-body p-0">
                        ...
                        <tbody id="tbody-cargafamiliar">
                            <?php if (!empty($cargafamiliar)): ?>
                            <?php foreach ($cargafamiliar as $cf): ?>
                                <tr class="text-center">
                                <td><?= $cf['nombre'] ?> <?= $cf['apepaterno'] ?> <?= $cf['apematerno'] ?></td>
                                <td><?= $cf['parentesco'] ?></td>
                                <td><?= date('d/m/Y', strtotime($cf['fechanac'])) ?></td>
                                <td><?= $cf['genero'] ?></td>
                                <td><?= $cf['estudia'] ? 'Sí' : 'No' ?></td>
                                <td><?= $cf['dependiente'] ? 'Sí' : 'No' ?></td>
                                <td>
                                    <?php if ($cf['evidencia']): ?>
                                    <a href="<?= base_url('uploads/evidencias/' . $cf['evidencia']) ?>" target="_blank" class="btn btn-sm btn-outline-secondary">
                                        Ver archivo
                                    </a>
                                    <?php else: ?>
                                    <span class="text-muted">—</span>
                            ...
                            <?php endforeach; ?>
                            <?php else: ?>
                            <tr><td colspan="7" class="text-center text-muted py-3">No hay cargas familiares registradas</td></tr>
                            <?php endif; ?>
                        </tbody>
                        </table>
                    </div>
                ...
                if (data.status === "ok") {
                    ...
                    data.cargafamiliar.forEach(familia => {
                       const tr = document.createElement("tr");
                        ...
                        tr.innerHTML = `
                            <td>${familia.nombre} ${familia.apepaterno} ${familia.apematerno}</td>
                            <td>${familia.parentesco}</td>
                            <td>${familia.fechanac ? new Date(familia.fechanac).toLocaleDateString() : '--'}</td>
                            <td>${familia.genero}</td>
                            <td>${familia.estudia ? 'Sí' : 'No'}</td>
                            <td>${familia.dependiente ? 'Sí' : 'No'}</td>
                            <td>${familia.evidencia ? `<a href='<?= base_url('uploads/evidencias/') ?>${familia.evidencia}' target='_blank' class='btn btn-sm btn-outline-secondary'>Ver archivo</a>` : '--'}</td>
                        `;
